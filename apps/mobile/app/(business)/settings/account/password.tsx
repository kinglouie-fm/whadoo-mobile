// apps/mobile/app/(business)/settings/account/password.tsx
...
export default function PasswordScreen() {
    ...
    const save = async () => {
        ...
        if (!newPassword || newPassword.length < 8) return Alert.alert("Weak password", "Please use at least 8 characters.");
        if (newPassword !== confirm) return Alert.alert("Mismatch", "New passwords do not match.");
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.bg }} edges={["top"]}>
            ...
            <View style={{ flex: 1, padding: 20, backgroundColor: theme.colors.bg, gap: 12 }}>
                {!canChange ? (
                    <Text style={{ color: theme.colors.muted }}>This account uses Google Sign-In. Password is managed by Google.</Text>
                ) : (
                    <>
                        <TextInput
                        ...
                        <Button title="Forgot password? Send reset email" onPress={doReset} />
                    </>
                )}
            </View>
        </SafeAreaView>

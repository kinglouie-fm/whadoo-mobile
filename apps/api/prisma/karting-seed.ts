...
const seeds: ActivitySeed[] = [
...

  {
    ...
    description: "Double Gold Race: 2× (11 min quali + 23 min race) (min 5 karts).",
    packages: [
      {
...
async function main() {
  if (!BUSINESS_ID || BUSINESS_ID.includes("PUT_YOUR_BUSINESS")) {
    throw new Error("Set BUSINESS_ID in prisma/karting-seed.ts before running.");
  }
  ...
  console.log(`✓ Seeded ${seeds.length} karting activities into group '${GROUP.catalogGroupId}'.`);
